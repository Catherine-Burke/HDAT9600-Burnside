...
#check which variables do not have a complete set of data
missing_cases <- ls(icu_patients_df1[,!complete.cases(t(icu_patients_df1))])
missing_cases
#check how many records are missing from each
colSums(is.na(icu_patients_df1[,missing_cases]))
#remove incomplete variables to maintain data integrity
ICUMort <- icu_patients_df1[,complete.cases(t(icu_patients_df1))]
#Add back SAPS1
ICUMort$SAPS1 <- icu_patients_df1$SAPS1
#Remove rows with missing values
ICUMort <- ICUMort[rowSums(is.na(ICUMort))<1,]
...
_SAPS1 score includes BUN, GCS, HCO3, HCT, HR, K, Na, RespRate, SysABP, Urine & WBC. Thee are 96 missing entries for SAPS1.  Could include this score_
...
ICUMort.f <- ICUMort[,c("Glucose_diff", "WBC_min", "pH_diff", "Bilirubin_min", "BUN_min", "PaO2_max", "RespRate_min", "MAP_max", "Na_max", "GCS_max", "Age", "Gender", "ICUType", "in_hospital_death", "SAPS1")]
...
full_mod<-glm(in_hospital_death ~ Glucose_diff + WBC_min + pH_diff +
                ...
                Age + Gender + ICUType + SAPS1, family = binomial(), data = ICUMort)
summary(full_mod)
...
full_mod_int <-glm(in_hospital_death ~ Glucose_diff + WBC_min + pH_diff +
                Albumin_max + BUN_min + RespRate_min + MAP_max + Na_max + GCS_max +
                Age + Gender + ICUType + SAPS1 + Age:Gender + Glucose_diff:Gender + BUN_min:Gender + 
                RespRate_min:Gender + Bilirubin_min:Gender + BUN_min:ICUType + PaO2_max:ICUType +
                Bilirubin_min:ICUType + GCS_max:ICUType, family = binomial(), data = ICUMort)
summary(full_mod_int)

#Compare to a null model
null_mod <- glm(in_hospital_death ~ 1, family = binomial, data = ICUMort)
...
red_int_mod <- step(full_mod_int, trace = 1)
...
###Jason multivariate
```{r, Task 1 multivariate models Jason}
#Include all variables from above
full_mod<-glm(in_hospital_death ~ Glucose_diff + WBC_min + pH_diff +
...
pred <- predict(red_int_mod , type = "response")
hl <- hoslem.test(red_int_mod$y, pred, g=80)
hl
#No indication of a lack of fit as p > 0.05.
#Create Brier score
bs <- mean((pred - ICUMort$in_hospital_death)^2)
bs
#Compare to full model
bs_full <- mean((predict(full_mod, type = "response") - ICUMort$in_hospital_death)^2)
bs_full
#Almost the same, more parsimonious model acceptible
#Compare to null model
bc_null <- mean((predict(null_mod, type = "response") - ICUMort$in_hospital_death)^2)
bc_null
...
PseudoR2(red_int_mod, which = "all")
#PsuedoR2 for full
