...
libs <- c("e1071", "learnr", "ggplot2", "tidyverse")
...
for (i in names(ICUMort.f)[1:12]){
  if(is.numeric(ICUMort.f[[i]])){
    ...
    plot_list2[[i]] <- ggplot(ICUMort.f, aes_string(x = "Gender", y=i)) + 
            ...
            labs(y=i, fill="Died", 
                 title = paste0("Density Plot of ", i,",\n",
                                length(na.omit(ICUMort.f[[i]]))," values available")) + theme(text = element_text(size=6))
      
          }
}
do.call("grid.arrange", c(plot_list2, ncol=nCol))
...
for (i in names(ICUMort.f[1:12])){
  if(is.numeric(ICUMort.f[[i]])){
    ...
    plot_list3[[i]] <- 
      ...
      geom_boxplot(outlier.colour = NA, aes(fill = factor(in_hospital_death, 
                                                labels = c("No","Yes")))) + 
      coord_cartesian(ylim = ybl) +
      labs(y=i, fill="Died", 
           title = paste0("Density Plot of ", i,",\n ",                                                length(na.omit(ICUMort.f[[i]])),
                          " values available")) + 
      theme(text = element_text(size = 6))
      
          }
}
do.call("grid.arrange", c(plot_list3, ncol = nCol))
...
plot_1 <-ggplot(ICUMort.f, aes(RespRate_min, GCS_max, colour = factor(in_hospital_death, labels = c("No","Yes")))) + geom_point(position = "jitter") + geom_smooth(method="lm") + labs(colour="Died")

#PaO2 not part of SAPS1, unexpected correlation
plot_2 <- ggplot(ICUMort.f, aes(PaO2_max, SAPS1, colour = factor(in_hospital_death, labels = c("No","Yes")))) + geom_point(position = "jitter") + geom_smooth(method="lm") + labs(colour="Died")

#GCS part of SAPS1 score - expected correlation
plot_3 <- ggplot(ICUMort.f, aes(GCS_max, SAPS1, colour = factor(in_hospital_death, labels = c("No","Yes")))) + geom_point(position = "jitter") + geom_smooth(method="lm") + labs(colour="Died")
...
unimod_summ
#Pull out P values
lapply(unimod_summ, function(x) {(coef(x)[,4])})
#All variables except MAP_max, WBC_max and Gender are significant in univariate models
...
Most variables which were significant in the univariate models appear to individually have a small effect on the risk of death.  Variables that increase the risk of death with aunit increase of the variable include SAPS1, Age, Bilirubin_diff, BUN_min, Platelets_diff and RespRate_min, with all increasing risk by between 0.1% - 13%. Variable that decrease risk were GCS_max (~16%), and PaO2_max (0.002%).

There are also many significant interactions between Age, Gender, ICUType and other variables.

##Fit Mutivariable Models

```{r, task 1 multivariate models}
...
red_full_mod$aic
sig_full_mod$aic
#test not significant, AIC similar, we can accept the simpler model with PaO2 removed - sig_full_mod
...
#possible models - sig_full_mod (f.mod1), red_int_mod (f.mod2), red_full_int_mod (f.mod3)
...
#(hl_null <- hoslem.test(null_mod$y, pred_null, g=80))
...
##
Based on the Hoslem test, Brier score and pseudoR2 values, models 2 (reduced interaction only model) and 3 (reduced model with variables + interactions) perform the best, with lower Brier scores and higher pseudoR2 values.  Hosmer_Lemeshow indicates no evidence for lack of fit.  Given that an interaction model only is very hard to interpret, model 3 is likely the best model.
...
plot.roc(oc, pred_mod1, auc.polygon = TRUE, 
         ...
         legacy.axes = TRUE)
plot.roc(oc, pred_mod2, auc.polygon = TRUE, 
         auc.polygon.col=rgb(.1,0.1,0.9, alpha = 0.3), col="blue", add=TRUE,
         legacy.axes = TRUE)
...
legend("bottomright",
       legend=c("Multivariable Model", "Interaction Model", 
                "Multivariable Interation Model", "SAPS Only Model"),
       title="Models:", 
       fill=c(rgb(.9,0.1,0.1, alpha = 0.3),
              rgb(.1,0.1,0.9, alpha = 0.3),
              rgb(.1,0.9,0.1, alpha = 0.3),
              #rgb(1,1,0.1, alpha = 0.3),
              rgb(0.1,1,1, alpha = 0.3)), 
       inset=c(0.1,0.1), cex = 0.5)
...
The multivariable model with no interactions (model 1) performs the worst with the smallest AUC, and lowest sensitivity and positive predictive value.  Models 2 (interactions only) and 3 (multivariable plus interactions) perform similarly.  Interestingly the interaction only model performs slightly better.  Both models all models perform better than a models based on SAPS1 alone, a previously developed predictor of mortality in hospital.
...
#Should start with univariate models for each of the variables we included.
#Create vector of variables
test.list.ICUtype <- test.list[test.list!="ICUType" & test.list!="in_hospital_death"]

#Create a list of univariate models for each of the variables selected
formulas<- paste('in_hospital_death ~', test.list.ICUtype)
...
CCU.full.mod <- glm(in_hospital_death ~ Age + Bilirubin_diff + BUN_max + GCS_max +
                      Gender + Glucose_max + MAP_min + Na_diff + PaO2_diff + pH_diff +
...
MICU.full.mod <- glm(in_hospital_death ~ Age + Bilirubin_min + BUN_min + GCS_min +
                       Gender + Glucose_min + MAP_min + Na_max + PaO2_min + pH_min +
                       Platelets_min + RespRate_max + TroponinI_max + WBC_diff + SAPS1 +
                       Age:Gender
...
SICU.full.mod <- glm(in_hospital_death ~ Age + Bilirubin_min + BUN_min + GCS_max + Gender +
                       Glucose_max + MAP_diff + Na_diff + PaO2_min + pH_diff + Platelets_max +
                       RespRate_min + TroponinI_max + WBC_diff + SAPS1 + Age:Gender
...
CSRU.full.mod <- glm(in_hospital_death ~ Age + Bilirubin_max + BUN_max + GCS_max + Gender + 
                       Glucose_min + MAP_min + Na_diff + PaO2_max + pH_max + Platelets_max +
                       RespRate_max + TroponinI_max + WBC_min + SAPS1 + Age:Gender
...
#CCU
CCU.sig.mod <- glm(in_hospital_death ~ BUN_max + GCS_max + PaO2_diff +
...
#fit better for significant model - keep
CCU.mod <- CCU.sig.mod

#MICU
MICU.sig.mod <- glm(in_hospital_death ~ Bilirubin_min + BUN_min + Gender + Na_max + TroponinI_max +
                       SAPS1 + Age:Gender, data=ICUMort.MICU, family="binomial")
...
#SICU
SICU.sig.mod <- glm(in_hospital_death ~ Age + BUN_min + GCS_max + Glucose_max + RespRate_min +
                      SAPS1 + Age:Gender, data=ICUMort.SICU, family="binomial")
...
#marginally better for reduced model
SICU.mod <- SICU.red.mod


CSRU.sig.mod <- glm(in_hospital_death ~ BUN_max + GCS_max + Na_diff + Platelets_max +
...
CSRU.full.mod$aic
CSRU.red.mod$aic
#not statistically different, AIC improved in reduced model. 
#compare brier scores
mean((predict(CSRU.sig.mod, type = "response") - ICUMort.CSRU$in_hospital_death)^2)
mean((predict(CSRU.red.mod, type = "response") - ICUMort.CSRU$in_hospital_death)^2)
#Keep red model.
CSRU.mod <- CSRU.red.mod
...
pred.CSRU <- predict(CSRU.sig.mod, type = "response")
(hl.CSRU <- hoslem.test(CSRU.sig.mod$y, pred.CSRU, g=40))
...
(bs.CSRU <- mean((pred.CSRU.red - ICUMort.CSRU$in_hospital_death)^2))
...
mean((ICUMort.t$predprob - ICUMort.t$in_hospital_death)^2)
mean((pred_null - ICUMort.t$in_hospital_death)^2)

#plot ROC for multi unit model
par(pty="s")
plot.roc(oc, ICUMort.t$predprob,auc.polygon = TRUE,
         auc.polygon.col=rgb(1,1,0.1, alpha = 0.3), col = "yellow",
...
plot.roc(oc, pred_mod3, auc.polygon = TRUE, 
         ...
         print.auc.y = 0.3)
#Add best model without any interactions
plot.roc(oc, pred_mod2, auc.polygon = TRUE, 
         auc.polygon.col=rgb(.1,0.1,0.9, alpha = 0.3), add=TRUE, col="blue", 
         legacy.axes=TRUE, print.auc = TRUE, print.auc.x = 0.3, 
         print.auc.y = 0.4)
legend("bottomleft",
       legend=c("Split ICU model", "Multivariable Interation Model", 
                "Interaction Model"),
       title="Models:", 
       fill=c(rgb(1,1,0.1, alpha = 0.3),
              rgb(.1,0.9,0.1, alpha = 0.3),
              rgb(.1,0.1,0.9, alpha = 0.3)), 
       inset=c(0.1,0.1), cex = 0.4)
```

```{r confusion matrix for best performing models}
#Calculate confusion matrix
#Use outcomes from seperate models for ICUtypes
pred_oc_ICUmods <-as.factor(ifelse(ICUMort.t$predprob < 0.5, "no", "yes"))
(ICUmods_cm <-confusionMatrix(pred_oc_ICUmods, ocf, positive = "yes"))
...
In general, for all the models tested the sensitivity was relatively low, and the positive predictive value somewhere between 50-60%.  This means that only ~20% of people that die would be identified as at high risk, and of the people identified as at high risk, about half of them are incorrectly classified.  Despite this, a single model (i.e. model 3) would still be useful if the aim was to predict which patients may be at higher risk of death and allocate resources accordingly.  In model 3 (multivariable + interactions), ICUType had the highest affect on risk of death in the model with risk of death increased by 600%!  However, in the multivariable model without interactions, SAPS1 has the strongest affect on risk of death, increasing by ~ 9% per unit increase of SAPS1.

However if the aim is to identify risk factors for death in hospital, the models split by ICU type would be more appropraiate, as patients in different ICU units may have different risk factors for death.
