...
```{r }
#code for density histograms and proportions

ICUMort <- readRDS("icu_patients_df1.rds")

for (i in names(ICUMort)){
  if(is.numeric(ICUMort[[i]])){
    
    print(ggplot(ICUMort, aes(ICUMort[[i]])) +
          #geom_density(aes(fill=factor(in_hospital_death, labels=c("No","Yes"))), alpha=.3) +
...
SAPS1 and SOFA seem like obvious choices, as these have been developed as predictors of clinical outcomes (SOFA) and risk of death (SAPS-I).  Age should also be included, as those who are very young or very old will likely have a higher risk of death.  SOFA includes scores for 
