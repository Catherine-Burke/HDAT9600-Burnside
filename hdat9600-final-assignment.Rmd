...
summary(icu_patients_df1[,c("Length_of_stay","SAPS1","SOFA","Survival","Age")])
...
## Yalin
I am a little confused about how to choose predictor variables for the model, but I found ranger function can be used to find the most important variable, don't know if it works
...
```{r Taks2_Part2.c}
fitbyall <- coxph( Surv(Days,Status) ~ . , data = hospitaldeath) 
summary(fitbyall)

library(ranger)
newdata <- na.omit(icu_patients_df1)
fitall <- ranger(Surv(Days, Status) ~ ., data=newdata, importance = "permutation")

fitalli <- data.frame(sort(round(fitall$variable.importance, 4), decreasing = TRUE))
names(vi) <- "importance"
fitalli
# As shown in the result, 5 most important variables: Survival, in_hospital_death, Bilirubin_min, Urine_max, Glucose_diff
...
fitbySurvival <- coxph( Surv(Days,Status) ~ Survival, data = icu_patients_df1) 
summary(fitbySurvival)

fitbydeath <- coxph( Surv(Days,Status) ~ in_hospital_death, data = icu_patients_df1) 
summary(fitbydeath)

fitbyBilirubinmin <- coxph( Surv(Days,Status) ~ Bilirubin_min, data = icu_patients_df1) 
summary(fitbyBilirubinmin)

fitbyUrinemax <- coxph( Surv(Days,Status) ~ Urine_max, data = icu_patients_df1) 
summary(fitbyUrinemax)

fitbyGlucosediff <- coxph( Surv(Days,Status) ~ Glucose_diff, data = icu_patients_df1) 
summary(fitbyGlucosediff)
