...
var_selection <- c("SOFA", "Glucose_diff", "WBC_min", "pH_diff", "Albumin_max", "BUN_min", "RespRate_min", "HR_diff", "Na_max", "Age", "Gender", "ICUType")
#Create a list of univariate models for each of the variables selected
formulas<- paste('in_hospital_death ~', var_selection)
unimod_list <- lapply(formulas, glm, family = binomial(), data = t.ICUMort)
...
full_mod<-glm(in_hospital_death ~ SOFA + Glucose_diff + WBC_min + pH_diff +
                Albumin_max + BUN_min + RespRate_min + HR_diff + Na_max +
                Age + Gender + ICUType, family = binomial(), data = t.ICUMort)
summary(full_mod)
#Compare to a null model
null_mod <- glm(in_hospital_death ~ 1, family = binomial, data = t.ICUMort)
...
red_mod <- glm(in_hospital_death ~ SOFA + pH_diff + Albumin_max + BUN_min + 
                 RespRate_min + HR_diff + Na_max +Age + Gender + ICUType, 
               family = binomial(), data = t.ICUMort)
summary(red_mod)
