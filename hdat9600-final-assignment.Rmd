...
ICUMort.f <- ICUMort[,c("SOFA", "Glucose_diff", "WBC_min", "pH_diff", "Albumin_max", "BUN_min", "RespRate_min", "HR_diff", "Na_max", "GCS_min", "Age", "Gender", "ICUType", "in_hospital_death")]
...
#further explore noted correlation between GCS_min and SOFA
ggplot(ICUMort.f, aes(GCS_min, SOFA, colour = factor(in_hospital_death, labels = c("No","Yes")))) + geom_point() + geom_smooth(method="lm") + labs(colour="Died")
...
#Create Brier score
bs <- mean((pred - t.ICUMort$in_hospital_death)^2)
bs
#Compare to full model
bs_full <- mean((predict(full_mod, type = "response") - t.ICUMort$in_hospital_death)^2)
bs_full
#Almost the same, more parsimonious model acceptible
#Compare to null model
bc_null <- mean((predict(null_mod, type = "response") - t.ICUMort$in_hospital_death)^2)
bc_null
...
oc <- as.factor(ifelse(t.ICUMort$in_hospital_death == 0, "no", "yes"))
