...
ICUMort.CCU <- ICUMort.t[ICUMort$ICUType=="Coronary Care Unit",]
ICUMort.CCU <- within(ICUMort.CCU, rm(ICUType))
ICUMort.MICU <- ICUMort.t[ICUMort$ICUType=="Medical ICU",]
ICUMort.MICU <- within(ICUMort.MICU, rm(ICUType))
ICUMort.SICU <- ICUMort.t[ICUMort$ICUType=="Surgical ICU",]
ICUMort.SICU <- within(ICUMort.SICU, rm(ICUType))
ICUMort.CSRU <- ICUMort.t[ICUMort$ICUType=="Cardiac Surgery Recovery Unit",]
...
print("AIC scores for selected models")
f.mod1$aic
f.mod2$aic
f.mod3$aic
...
plot.roc(oc, pred_mod2, auc.polygon = TRUE, 
         ...
         print.auc.y = 0.4)
legend("bottomleft",
       legend=c("Split ICU model", "Multivariable Interation Model", "Interaction Model"),
       title="Models:", 
       fill=c(rgb(1,1,0.1, alpha = 0.3),rgb(.1,0.9,0.1, alpha = 0.3),rgb(.1,0.1,0.9, alpha = 0.3)), 
       inset=c(0.1,0.1))
...
summary(df0.f)
val.mod1 <- glm(in_hospital_death ~ Age + GCS_max + SAPS1 + ALP_max + 
                PaO2_max + Urine_max + BUN_min + ICUType + Lactate_diff, 
                family = binomial(), data = df0.f)
val.mod2 <- glm(in_hospital_death ~ HR_min:GCS_max + ALP_max:Gender + 
                Gender:BUN_min + Gender:Lactate_diff + Age:ICUType + GCS_max:ICUType + 
                ICUType:SAPS1 + ICUType:I(HR_min^2) + ICUType:PaO2_max + 
                ICUType:Urine_max + BUN_min:ICUType + Gender:ICUType, family = binomial(), 
                data = df0.f)
...
CCU.val.mod <- glm(in_hospital_death ~ BUN_min + GCS_max + SAPS1 + 
    Age:Gender, family = "binomial", data = df0.f[df0.f$ICUType=="Coronary Care Unit",])

MICU.val.mod <- glm(in_hospital_death ~ Age + SAPS1 + HR_min + ALP_max + 
    Urine_max + BUN_min + Lactate_diff + Gender, family = "binomial", 
    data = df0.f[df0.f$ICUType=="Medical ICU",])

SICU.val.mod <- glm(in_hospital_death ~ Age + BUN_min + GCS_max + SAPS1 + 
    Age:Gender, family = "binomial", data = df0.f[df0.f$ICUType=="Surgical ICU",])

CSRU.val.mod <- glm(in_hospital_death ~ GCS_max + PaO2_max + Urine_max + 
    BUN_min + Lactate_diff + Age:Gender, family = "binomial", 
    data = df0.f[df0.f$ICUType=="Cardiac Surgery Recovery Unit",])

#create column for predicted probability
df0.f$predprob <- 0
#populate probability for individual unit types across whole data set
df0.f[df0.f$ICUType=="Coronary Care Unit",] %>% mutate(predprob=predict(CCU.val.mod, type="response")) -> df0.f[df0.f$ICUType=="Coronary Care Unit",]
df0.f[df0.f$ICUType=="Medical ICU",] %>% mutate(predprob=predict(MICU.val.mod, type="response")) -> df0.f[df0.f$ICUType=="Medical ICU",]
df0.f[df0.f$ICUType=="Surgical ICU",] %>% mutate(predprob=predict(SICU.val.mod, type="response")) -> df0.f[df0.f$ICUType=="Surgical ICU",]
df0.f[df0.f$ICUType=="Cardiac Surgery Recovery Unit",] %>% mutate(predprob=predict(CSRU.val.mod, type="response")) -> df0.f[df0.f$ICUType=="Cardiac Surgery Recovery Unit",]


#plot ROC for multi unit model
par(pty="s")
plot.roc(df0.f$in_hospital_death, df0.f$predprob,auc.polygon = TRUE,
         auc.polygon.col=rgb(1,1,0.1, alpha = 0.3), col = "yellow",
         legacy.axes = TRUE, print.auc = TRUE, 
         print.auc.y = 0.2, print.auc.col = "orange")

#Compare to best model for all ICU units
plot.roc(df0.f$in_hospital_death, predict(val.mod1, type="response"), auc.polygon = TRUE, 
         auc.polygon.col=rgb(.9,0.1,0.1, alpha = 0.3),add=TRUE, col="green",
         legacy.axes = TRUE, print.auc = TRUE, print.auc.x = 0.4, 
         print.auc.y = 0.3)
#Compare to best model for all ICU units
plot.roc(df0.f$in_hospital_death, predict(val.mod2, type="response"), auc.polygon = TRUE, 
         auc.polygon.col=rgb(.1,0.9,0.1, alpha = 0.3),add=TRUE, col="green",
         legacy.axes = TRUE, print.auc = TRUE, print.auc.x = 0.4, 
         print.auc.y = 0.3)
#Add best model without any interactions
plot.roc(df0.f$in_hospital_death, predict(val.mod3, type="response"), auc.polygon = TRUE, 
         ...
         print.auc.y = 0.4)
legend("bottomleft",
       legend=c("Split ICU model", "Multivarialable Model","Multivariable Interaction Model", "Interaction Model"),
       title="Models:", 
       fill=c(rgb(1,1,0.1, alpha = 0.3),rgb(.9,0.1,0.1, alpha = 0.3),rgb(.1,0.9,0.1, alpha = 0.3),rgb(.1,0.1,0.9, alpha = 0.3)), 
       inset=c(0.1,0.1))
...
pred_oc_ICUmods <-as.factor(ifelse(ICUMort.f$predprob < 0.5, "no", "yes"))
